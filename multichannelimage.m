classdef multichannelimage < matlab.mixin.Copyable
    ...
    methods
        ...
        function makeDolan(obj, baseSet, targetSet, numPoints, netColoc, netMulti, netColocRandom, netMultiRandom, tolerance)
            ...
            dolDolan = dolan(baseSet, targetSet, 'DOL over threshold', DOL, [], 'parameter: distance threshold');
            multiDolan = dolan(baseSet, targetSet, 'multi-assignments over threshold', netMulti, [], 'parameter: distance threshold');
            dolRandomDolan = dolan(baseSet, targetSet, 'DOL-Random over threshold', DOLrandom, [], 'parameter: distance threshold');
            multiRandomDolan = dolan(baseSet, targetSet, 'multi-assignments-Random over threshold', netMultiRandom, [], 'parameter: distance threshold');
        ...
        function makeSignificantDolan(obj, baseName, targetName, significantT)
            ...
            sDOL = dolan(baseSet, targetSet, 'DOL', dolDolan.value(sIndex), [], sprintf('threshold: %1.1f px', significantT));
            sMulti = dolan(baseSet, targetSet, 'multi-assignments', multiDolan.value(sIndex), [], sprintf('threshold: %1.1f px', significantT));
            sDOLRand = dolan(baseSet, targetSet, 'DOL-Random', dolRandomDolan.value(sIndex), [], sprintf('threshold: %1.1f px', significantT));
            sMultiRand = dolan(baseSet, targetSet, 'multi-assignments-Random', multiRandomDolan.value(sIndex), [], sprintf('threshold: %1.1f px', significantT));
