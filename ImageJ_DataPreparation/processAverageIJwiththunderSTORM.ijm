//J Euchner//K Yserentant//SA Tashev//
...
for (i=0; i<list.length; i++) {
	...
	showProgress(i-1, dir_list.length);
		...
		for(j=0; j<subdir_files.length; j++) {
			if (endsWith(subdir_files[j], ".tif")) {
				...
				if (endsWith(name, ""+reference+".tif")) {
	...
				else if (endsWith(name,""+HaloTag+".tif")) {
	...
				else if (endsWith(name, ""+SNAPtag+".tif")) {
