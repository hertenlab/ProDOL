classdef imageset < matlab.mixin.Copyable
    ...
    properties
        ...
        results = dolan.empty;              % dolan object vector of DOL analysis results
    ...
    methods
        ...
        function calculateMeanTransformation(obj, baseName, targetName)
            ...
            count = 0;
            success = 0;
            includedPointSets = pointset.empty;
            for i = 1:length(obj)
                for j = 1:length(obj(i).childImages)
                    count = count + 1;
                    if ~isempty(obj(i).childImages(j).pointSetByName(targetName).transformation)
                        ...
                        meanT = mean(cat(3,meanT,thisT),3);
                        success = success + 1;
                    end
                end
            end
        ...
        function colocalisation(obj, baseName, targetName)
...
%             obj.calculateColocOverThresholds(baseName, targetName)
            ...
            obj.findSignificantThreshold(baseName, targetName);
        ...
        function calculateColocOverThresholds(obj, baseName, targetName)
            ...
            for i = 1:length(obj)
                ...
                parfor (j = 1:length(orphans), 4)            % TODO change back to parfor
                    thisImage = orphans(j);
        ...
        function findSignificantThreshold(obj, baseName, targetName)
            ...
            distanceThreshold = colocalisationThreshold(dolValues, randomValues, thresholds, 'plot');
            % store distance threshold as dolan with imageset
            colocDolan = dolan(baseName, targetName, 'colocThreshold', distanceThreshold, [], '');
        ...
        function calculateMeanColocalisation(obj, selector, baseName, targetName)
            ...
            for i = 1:length(obj)
                dispProgress(i, length(obj))
                obj(i).makeMeanDolan(selector, baseName, targetName, 'DOL', 'mean DOL');
                obj(i).makeMeanDolan(selector, baseName, targetName, 'DOL-Random', 'mean DOL Random');
                obj(i).makeMeanDolan(selector, baseName, targetName, 'multi-assignments', 'mean multi-assignments');
                obj(i).makeMeanDolan(selector, baseName, targetName, 'multi-assignments-Random', 'mean multi-assignments Random');
            end
        end
        
        % calculate mean over images of this set and store as results dolan
        % in this imageset.
        function makeMeanDolan(obj, selector, baseName, targetName, varName, resultName)
            ...
            meanValue = nanmean(valVector);
            stdValue = nanstd(valVector);
            % append (include) or (exclude) to resultName (if
            % applicable)
            if ~strcmp(selector, 'all')
                resultName = [resultName ' (' selector ')'];
            end
            resultDolan = dolan(baseName, targetName, resultName, meanValue, stdValue, 'uncertainty: standard deviation');
        ...
        function calculateMeanDensity(obj, selector, setName)
            for i = 1:length(obj)
                ...
                meanDensity = nanmean(valVector);
                stdDensity = nanstd(valVector);
                % append (include) or (exclude) to resultName (if
                % applicable)
                if strcmp(selector, 'all')
                    resultName = 'mean Density';
                else
                    resultName = ['mean Density (' selector ')'];
                end
                resultDolan = dolan(setName, [], resultName, meanDensity, stdDensity, 'uncertainty: standard deviation');
        ...
        function calculateAllMeanDensities(obj)
            for i = 1:length(obj)
                ...
                includeVector = cell(length(obj(i).childImages));
                % retrieve all density values
                for j = 1:length(obj(i).childImages)
                    mci = obj(i).childImages(j);
                    includeVector{i} = mci(i).include;
                    for k = 1:numPtSets
                ...
                for k = 1:numPtSets
                    ...
                    resultDolan = dolan(setNames{k}, [], 'mean Density', meanDensity, stdDensity, 'uncertainty: standard deviation');
                    obj(i).addResults(resultDolan);
                    % include
                    idx = cellfun(@any, includeVector);
                    meanDensity = nanmean(valVector(idx,k));
                    stdDensity = nanstd(valVector(idx,k));
                    resultDolan = dolan(setNames{k}, [], 'mean Density (include)', meanDensity, stdDensity, 'uncertainty: standard deviation');
                    obj(i).addResults(resultDolan);
                    % exclude
                    idx = ~cellfun(@any, cellfun(@not, includeVector, 'UniformOutput', false));
                    meanDensity = nanmean(valVector(idx,k));
                    stdDensity = nanstd(valVector(idx,k));
                    resultDolan = dolan(setNames{k}, [], 'mean Density (exclude)', meanDensity, stdDensity, 'uncertainty: standard deviation');
                    obj(i).addResults(resultDolan);
                end
            end
        end
        ...
        function addColocThreshold(obj, newDolan)
            ...
            for i = 1:length(obj)
                ...
                if ~isa(obj(i).colocThreshold, 'dolan')
                    obj(i).colocThreshold = dolan.empty;
                end
