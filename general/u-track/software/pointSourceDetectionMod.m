...
function [pstruct, mask, imgLM, imgLoG] = pointSourceDetection(img, sigma, popthreshold, varargin)
...
% _______own code: intenstity threshold____________
popthreshold;
stepsize=5;
AllMaxHist=reshape(allMax, [512*512,1]);
AllMaxHistSmall=(AllMaxHist(AllMaxHist>0));
[countx,centerx]=histcounts(AllMaxHistSmall,[0:stepsize:300]);
% figure()
% plot(centerx(1:length(countx)),countx)
% xlim([0 300])
MaxPos=find(countx==max(countx));
index=max(MaxPos); %max is neccessary if there are two peaks with the same hight, then choose the one on the right
while index*stepsize<300 && countx(index)>(countx(max(MaxPos))/popthreshold)%<- the popthreshold (default=50) here is the relative intensity threshold
    index=index+1;
    limit=index*stepsize;   
end
limit=mean(limit)
% the limit equals to an intensity at the right end of the background
% intensity distribution (namely 1/50 of the maximal counts)
for i=1:512
    for j=1:512
        if allMax(i,j)<limit
                allMax(i,j)=0;
...
% _______end own code______________________________
