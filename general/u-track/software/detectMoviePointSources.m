...
% %         %start own piece of code to load mask
% %         
% %         %segmentation mask from mask channel
        [~, chNames] = cellfun(@fileparts,{movieData.channels_.channelPath_},'UniformOutput',false);
        maskIdx = find(strcmp(chNames,'mask'));
        detP.Mask = double(movieData.channels_(maskIdx).loadImage(j)); %channel(4) is the mask, when all 5 channels are loaded (bleached,blue,green,mask,red)
%
