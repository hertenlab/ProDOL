...
%   it for multiple threshold define vector (e.g. (0.1:0.1:4) or [1.5 1.6 1.7])
% - prctile_RedGreen
%   Percentage of point amplitudes in unstained sample to define amplitude
%   threshold for false positives (e.g. 50 (for median), 75, 90). If set to
%   0 threshold is calculated from mean+std amplitude of unstained sample
% - thresholdA_Blue
...
function filterPointsDOL_rgb(matfilepath, thresholdColoc, prctile_RedGreen, thresholdA_Blue)

load(matfilepath);

wb = waitbar(0, 'Step 1: Initialize Variables', 'Name', 'RegistrationRoutine');

%% Filter points by amplitude
...
for ct = 1:2
for t = 1:5
    exp_index = strcmp(CellType, Cells{ct}) & ...
        concentration == 0 & ...
        incubation_time == inctime(t);
    if prctile_RedGreen == 0
    min_A.Green(t,ct) = mean([Points_Green_A{exp_index}]) + (std([Points_Green_A{exp_index}]));
    min_A.Red(t,ct) = mean([Points_Red_A{exp_index}]) + (std([Points_Red_A{exp_index}]));
    else 
    min_A.Green(t,ct) = prctile([Points_Green_A{exp_index}], prctile_RedGreen);
    min_A.Red(t,ct) = prctile([Points_Red_A{exp_index}], prctile_RedGreen);
    end
end
end
min_A.Blue(1:5,1:2) = thresholdA_Blue;
...
index_Green{i} = Points_Green_A{i} > min_A.Green(x,y);
...
index_Red{i} = Points_Red_A{i} > min_A.Red(x,y);
...
index_Blue{i} = Points_Blue_A{i} > min_A.Blue(x,y);
