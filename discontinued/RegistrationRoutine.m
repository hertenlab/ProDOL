function RegistrationRoutine(movieListPath)

if not(nargin)
    movieListPath = 'y:\DOL Calibration\Data\felix\u-track\movieList_15-30-60auto-3h-overnight.mat';
end
...
if isempty(Points_Blue_x{i}) || length(Points_Blue_x{i}) < 10
    ...
    if isempty(Points_Red_x{i}) || length(Points_Red_x{i}) < 10
        warning(['Not enough points for registration on cell ' num2str(i)...
        ' in Red Channel']);
        FlagRed{i} = 'Registration might not be reliable';
    else
    ...
    if isempty(Points_Green_x{i}) || length(Points_Green_x{i}) < 10
        warning(['Not enough points for registration on cell ' num2str(i)...
        ' in Green Channel']);
        FlagGreen{i} = 'Registration might not be reliable';
    else
